...
int main(int argc, char** argv){
    ...
    upper = (myid > 0) ? (myid - 1) : MPI_PROC_NULL;
    lower = (myid < 3) ? (myid + 1) : MPI_PROC_NULL;
    for (int n = 0; n < steps; ++n) {
        // 向上接收/向下发送
            MPI_Sendrecv( &a[1][0] , totalsize , MPI_DOUBLE , upper , tag1 , &a[mysize + 1][0] , totalsize , MPI_DOUBLE , lower , tag1 , MPI_COMM_WORLD , &status);
        // 向下接收/向上发送
            MPI_Sendrecv( &a[mysize][0] , totalsize , MPI_DOUBLE , upper , tag2 , &a[0][0] , totalsize , MPI_DOUBLE , lower , tag2 , MPI_COMM_WORLD , &status);
